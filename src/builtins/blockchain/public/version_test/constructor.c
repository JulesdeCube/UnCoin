...
void blockchain_init_bufferHash(Block block, int increment_nonce)
{
    ...
    char *previousHash = blockchain_hash_str(block->previousHash);
    char *nonce_str = malloc(sizeof(char) * (nonce_comp(block->nonce) + strlen(previousHash)));

    sprintf(nonce_str, "%ld", block->nonce);

    nonce_str = strcat(nonce_str, (char *)block->data);
    nonce_str = strcat(nonce_str, previousHash);
