...
int block_isValid(Block block)
{
    ...
    int error_validNonce = PoW_isValidNonce(block, message);
...
void PoW_findNonce(Block block, char *message)
{
    while (PoW_isValidNonce(block, message) != 1)
        block->nonce++;
}
...
int PoW_isValidNonce(Block block, char *message)
{
